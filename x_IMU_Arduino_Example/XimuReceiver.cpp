...
ErrorCode XimuReceiver::processNewChar(unsigned char c) {
    ...
    if(c & 0x80) {
        ...
        packet[7]  = (buf[7]  << 1) | (buf[8]  >> 6);
        packet[8]  = (buf[8]  << 2) | (buf[9]  >> 5);
        packet[9]  = (buf[9]  << 3) | (buf[10] >> 4);
        packet[10] = (buf[10] << 4) | (buf[11] >> 3);
        packet[11] = (buf[11] << 5) | (buf[12] >> 2);
        packet[12] = (buf[12] << 6) | (buf[13] >> 1);
        packet[13] = (buf[13] << 7) | (buf[14] >> 0);
        packet[14] = (buf[14] << 1) | (buf[15] >> 6);
        packet[15] = (buf[15] << 2) | (buf[16] >> 5);
        packet[16] = (buf[16] << 3) | (buf[17] >> 4);
        packet[17] = (buf[17] << 4) | (buf[18] >> 3);
        packet[18] = (buf[18] << 5) | (buf[19] >> 2);
        packet[19] = (buf[19] << 6) | (buf[20] >> 1);
        packet[20] = (buf[20] << 7) | (buf[21] >> 0);
        packet[21] = (buf[21] << 1) | (buf[22] >> 6);
        packet[22] = (buf[22] << 2) | (buf[23] >> 5);
        packet[23] = (buf[23] << 3) | (buf[24] >> 4);
        packet[24] = (buf[24] << 4) | (buf[25] >> 3);
        packet[25] = (buf[25] << 5) | (buf[26] >> 2);
        packet[26] = (buf[26] << 6) | (buf[27] >> 1);
        packet[27] = (buf[27] << 7) | (buf[28] >> 0);
